            ...
            else
            {
                ...
                while (!sr.EndOfStream)
                {
                    ...
                    if (idx == -1)
                    {
                        ...
                        movie.id = movieDetails[0];
                        movie.title = movieDetails[1];
                        genres = movieDetails[2];
                        movie.director = movieDetails.Length > 3 ? movieDetails[3] : "unassigned";
                        movie.runtime = movieDetails.Length > 4 ? TimeSpan.Parse(movieDetails[4]) : new TimeSpan(0);
                    }
                    else
                    {
                        // quote = comma or quotes in movie title
                        // extract the movieId
                        movie.id = line.Substring(0, idx - 1);
                        ...
                        movie.runtime = details.Length > 2 ? TimeSpan.Parse(details[2]) : new TimeSpan(0);
                    }
                    sw.WriteLine($"{movie.id},{movie.title},{genres},{movie.director},{movie.runtime}");
                }
                sw.Close();
                sr.Close();
